...
int modularity1(int N, vector< vector< tuple<int, int, char> >> links, double InitTemp, double CR, int TL, int Max_Success) {
	...
	for (int k = min_k; k <= max_k; k++)
	{
		...
		while (IT > LimitIT)
		{
			...
			/*NOTE: Z_to[i] is the sum of the edge weights FROM vertex l_star TO the set of vertices in cluster i
...
/*
	...
	bestzsol << "MODULARITY1_SOLUTION_CLUSTERING" << ".txt";
	bestzstorage.open("MODULARITY1_SOLUTION_CLUSTERING.txt", ios::app);
	for (int i = 0; i < final_membership.size(); i++) { bestzstorage << final_membership[i] << ",";}
	bestzstorage << endl;
	bestzstorage.close();
	std::cout << "The membership vector for this clustering has been written to MODULARITY1_SOLUTION_CLUSTERING.txt." << endl;
*/
	ostringstream bestzsol_BIC;
	ofstream bestzstorage_BIC;
	// If old file exists from a previous run, delete it.
	remove("MODULARITY1_SOLUTION_CLUSTERING_BIC.txt");
	bestzsol_BIC << "MODULARITY1_SOLUTION_CLUSTERING_BIC" << ".txt";
	bestzstorage_BIC.open("MODULARITY1_SOLUTION_CLUSTERING_BIC.txt", ios::app);
	...
	std::cout << "The membership vector for this clustering has been written to MODULARITY1_SOLUTION_CLUSTERING_BIC.txt." << endl;

	ostringstream bestzsol_AIC;
	ofstream bestzstorage_AIC;
	// If old file exists from a previous run, delete it.
	remove("MODULARITY1_SOLUTION_CLUSTERING_AIC.txt");
	bestzsol_AIC << "MODULARITY1_SOLUTION_CLUSTERING_AIC" << ".txt";
	bestzstorage_AIC.open("MODULARITY1_SOLUTION_CLUSTERING_AIC.txt", ios::app);
	...
	std::cout << "The membership vector for this clustering has been written to MODULARITY1_SOLUTION_CLUSTERING_AIC.txt." << endl;
