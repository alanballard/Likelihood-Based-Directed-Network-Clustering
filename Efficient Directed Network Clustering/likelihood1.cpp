...
//Total weight of links from vertices of cluster * to vertex l* (These are the Z_(from *) variables in the paper)
		...
		while (IT > LimitIT)
		{
			...
			if (first_run == 1)
			{
				...
				//Thus the initial clustering and associated likelihood are assigned to the best_likelihood
...
//cin.get();
			...
			else
			{
				...
				long double min_val = min((long double)1, exp(delta_l / (IT)));//delta_l=l_1-l_0<0 -> l_1<l_0
...
//cout << "uni_draw=" << uni_draw << ", min_val=" << min_val << ", -delta_l="<<-delta_l<<", IT="<<IT<<"-delta_l/IT="<<-delta_l/IT<< "exp(-delta_l / (IT))="<<exp(-delta_l / (IT))<<endl;
...
//std::cout << k << "..." << overall_iteration_count <<"..."<< STG1_CT << "..." << STG2_CT << "..." << STG3_CT << "..." << endl;;

//When initial temp=100, we accept everything because min_val is very, very close to 1.
//When initial temp=1, we accept virtually nothing. Out of say, the first 4000 attempts, 30 will be accepted and 3970 will be rejected.
//Maybe need to find a smoother acceptance function?
...
*/
	...
	ofstream stats_file;
	stats_name << "LIKELIHOOD1_STATS" << ".txt";
	stats_file.open("LIKELIHOOD1_STATS.txt", ios::app);
...
//remove("LIKELIHOOD1_SOLUTION_CLUSTERING_BIC.txt");
	bestzsol_BIC << "LIKELIHOOD1_SOLUTION_CLUSTERING_BIC" << ".txt";
	bestzstorage_BIC.open("LIKELIHOOD1_SOLUTION_CLUSTERING_BIC.txt", ios::app);
...
//	std::cout << "The membership vector for this clustering has been written to LIKELIHOOD1_SOLUTION_CLUSTERING_BIC.txt." << endl;
	
	ostringstream bestzsol_AIC;
	ofstream bestzstorage_AIC;
	// If old file exists from a previous run, delete it.
//remove("LIKELIHOOD1_SOLUTION_CLUSTERING_AIC.txt");
	bestzsol_AIC << "LIKELIHOOD1_SOLUTION_CLUSTERING_AIC" << ".txt";
	bestzstorage_AIC.open("LIKELIHOOD1_SOLUTION_CLUSTERING_AIC.txt", ios::app);
...
//	std::cout << "The membership vector for this clustering has been written to LIKELIHOOD1_SOLUTION_CLUSTERING_AIC.txt." << endl;
...
for (int row = 0; row < k; row++) {
for (int col = 0; col < k; col++) {
if((row==clust_l||col==clust_l)||(row == clust_j || col == clust_j))
{
if (OBS[row][col] == 0 || OBS[row][col] == POS[row][col]) { l_0 = l_0; }
else {
l_0 = l_0 + (OBS[row][col])*log(OBS[row][col] / (long double)POS[row][col]) + (POS[row][col] - OBS[row][col])*log(1 - (OBS[row][col] / (long double)POS[row][col]));
}
}
}
}
...
/*
l_1 = 0;
//Get  numerator of change-in-loglikelihood equation for proposed clustering
for (int row = 0; row < k; row++) {
for (int col = 0; col < k; col++) {
if ((row == clust_l || col == clust_l) || (row == clust_j || col == clust_j))
{
if (OBS_TEMP[row][col] == 0 || OBS_TEMP[row][col] == POS_TEMP[row][col]) { l_1 = l_1; }
else {
l_1 = l_1 + (OBS_TEMP[row][col])*log(OBS_TEMP[row][col] / (long double)POS_TEMP[row][col]) + (POS_TEMP[row][col] - OBS_TEMP[row][col])*log(1 - (OBS_TEMP[row][col] / (long double)POS_TEMP[row][col]));
}
}
}
}
