...
//		std::cout << "Now evaluating k=" << k << endl;
		...
		while (IT > LimitIT)
		{
			...
			if (first_run == 1)
			{
				...
				//Get total number of observed between-cluster edges
				int BW_OBS = 0;
				//Get total number of possible between-cluster edges
				int BW_POS = 0;
				for (int row = 0; row < k; row++) {
					for (int col = 0; col < k; col++) {
						if(row!=col){ 
							BW_OBS = BW_OBS + OBS[row][col]; 
							BW_POS = BW_POS + POS[row][col];
						}						
					}
				}
			...
			for (int row = 0; row < k; row++) {
				for (int col = 0; col < k; col++) {
					if (row != col) {
						BW_OBS = BW_OBS + OBS_TEMP[row][col];
						BW_POS = BW_POS + POS_TEMP[row][col];
					}
				}
			}
...
*/
	...
	ofstream stats_file;
	stats_name << "LIKELIHOOD20_STATS" << ".txt";
	stats_file.open("LIKELIHOOD20_STATS.txt", ios::app);
...
//remove("LIKELIHOOD20_SOLUTION_CLUSTERING_BIC.txt");
	bestzsol_BIC << "LIKELIHOOD20_SOLUTION_CLUSTERING_BIC" << ".txt";
	bestzstorage_BIC.open("LIKELIHOOD20_SOLUTION_CLUSTERING_BIC.txt", ios::app);
...
//	std::cout << "The membership vector for this clustering has been written to LIKELIHOOD20_SOLUTION_CLUSTERING_BIC.txt." << endl;

	ostringstream bestzsol_AIC;
	ofstream bestzstorage_AIC;
	// If old file exists from a previous run, delete it.
//remove("LIKELIHOOD20_SOLUTION_CLUSTERING_AIC.txt");
	bestzsol_AIC << "LIKELIHOOD20_SOLUTION_CLUSTERING_AIC" << ".txt";
	bestzstorage_AIC.open("LIKELIHOOD20_SOLUTION_CLUSTERING_AIC.txt", ios::app);
...
//	std::cout << "The membership vector for this clustering has been written to LIKELIHOOD20_SOLUTION_CLUSTERING_AIC.txt." << endl;
