...
//ALAN			cout<<"it took too long to decide the memberships; I will try to change the community sizes"<<endl;

//ALAN			cout<<"new community sizes"<<endl;
//ALAN			for (int i=0; i<num_seq.size(); i++)
//ALAN			cout<<num_seq[i]<<" ";
//ALAN			cout<<endl<<endl;
...
//ALAN			cout<<"degree sequence changed to respect the option -sup ... "<<++eras_add_times<<endl;
...
//ALAN			cout<<"degree sequence changed to respect the option -inf ... "<<++eras_add_times<<endl;
...
out2 << i+1  << "\t";
		for (int j=0; j<member_list[i].size(); j++)
{ 
//			out2<<member_list[i][j]+1<<" ";  ALAN
			out2 << member_list[i][j] + 1;
			//if (i < member_list.size()-1) { //ALAN
			//	out2 << "," <<" ";//ALAN
			//}//ALAN
//out2 << member_list[i][j] + 1 ;
...
//Solutions file for big batch run. Each network's solution will be comma-seperated, on a seperate line
	std::ofstream ofs;
	ofs.open("NETWORK_SOLUTIONS.dat", std::ofstream::out | std::ofstream::app);
	for (int i = 0; i<member_list.size(); i++) {		
		//ofs << i + 1 << "\t";
		for (int j = 0; j<member_list[i].size(); j++)
		{			
			ofs << member_list[i][j] + 1;
...
//ALAN	cout<<"network of "<<num_nodes<<" vertices and "<<edges<<" edges"<<";\t average degree = "<<double(edges)/num_nodes<<endl;
//ALAN	cout<<"\naverage mixing parameter (in-links): "<<average_func(double_mixing_in)<<" +/- "<<sqrt(variance_func(double_mixing_in))<<endl;
//ALAN	cout<<"average mixing parameter (out-links): "<<average_func(double_mixing_out)<<" +/- "<<sqrt(variance_func(double_mixing_out))<<endl;
//ALAN	cout<<"p_in: "<<density<<"\tp_out: "<<sparsity<<endl;

	
	
	ofstream statout("LFR_statistics.dat");
...
//ALAN		cout<<"-----------------------------------------------------------"<<endl;
//ALAN		cout<<"community size range automatically set equal to ["<<nmin<<" , "<<nmax<<"]"<<endl;
...
//ALAN	cout<<"building communities... "<<endl;
...
//ALAN	cout<<"connecting communities... "<<endl;
...
//	Parameters p;
	...
	*/
	...
	bool excess = false;
	bool defect = false;
	//int num_nodes = 50;//N		number of nodes
	//double average_k = 5;//k		average in-degree
	//int max_degree = 10;//maxk	maximum in-degree
	//double tau = 3;//t1		minus exponent for the degree sequence
	//double tau2 = 1;//t2		minus exponent for the community size distribution
	//double mixing_parameter = 0.2;//mu		mixing parameter
	int overlapping_nodes = 0;//on		number of overlapping nodes
	int overlap_membership = 0;//om		number of memberships of the overlapping nodes
	//int nmin = 5;//minc	minimum for the community sizes
	//int nmax = 10;//maxc	maximum for the community sizes
